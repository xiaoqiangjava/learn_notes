...
###### 任务推测执行
* 发现拖后腿的任务，比如某个任务的运行速度远慢于任务平均运行速度，那么为拖后腿的任务启动一个备份任务，同时运行。谁先运行完，采用谁的结果。
* 每个task只能由一个备份任务
* 当前job已经完成task必须不小于5%
* 配置方法：
		// 修改mapred-site.xml, 默认已经开启了任务推测执行
		<property>
...
* 推测执行算法原理：
		...
		// 推测备份任务完成时间
		备份任务推测完成时间（16） = 当前时刻（6）- 任务运行平均时间（10）
* MapReduce总是选择（推测执行完成时间）-（备份任务推测执行完成时间）差值最大的任务，并为之启动备份任务。
* 为了防止大量任务同时启动备份任务造成集群资源的浪费，MR为每个job设置了同时启动的备份任务数目上限。
* 推测运行机制实际上使用了经典的算法：空间换时间，他同时启动多个任务处理相同的数据，并让这行任务竞争，以缩短任务执行时间。显然这种方法需要占用更多的计算资源，在集群资源紧缺的情况下，应合理应用该机制。争取在多用少量资源的情况下，缩短任务运行的时间。
...
* 减少数据倾斜的方法
	...
	* Combiner：使用Combine可以大量地减小数据倾斜。在可能的情况下，combine的目的就是聚合并精简数据。
